...
def formatear_con_coma_miles(valor):
    """
    Formatea un número con coma como separador de miles y sin decimales.
    Ej: 2500.0 → '2,500'
    """
    try:
        valor_int = int(round(valor))
        return f"{valor_int:,}"  # Coma como separador de miles
    except Exception:
        return valor

def obtener_servicios_por_empresa(request):
    """
    Devuelve una lista de servicios en formato JSON para una empresa específica.
    Incluye nombre, descripción, precio formateado y duración.
    """
